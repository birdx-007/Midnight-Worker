...
{
    public MapEnemyData(int X, int Y, EnemyType type, List<Vector2Int> points = null)
    {
        enemyType = type;
        waypoints = points;
        posX = X;
...
public class Map : ISerializationCallbackReceiver
{
    ...
    public void CreateEnemy(int X, int Y, EnemyType type, List<Vector2Int> points = null)
    {
        if (!IsOccupied(X, Y))
        {
            mapArray[X + mapCenter.x, Y + mapCenter.y] = -2;
        }
        mapEnemyList.Add(new MapEnemyData(X, Y, type, points));
    }
}